...
/* 呼吸灯效果线程 */
static void breathing_thread_entry(void* parameter)
{
    uint8_t color_data[WS2812_LED_NUM * 3];
    float angle = 0.0f;
    ...
    while (1) {
        ...
        float brightness_factor = (sinf(angle) + 1.0f) / 2.0f;  // 将-1到1的正弦值映射到0到1
        uint8_t current_brightness = (uint8_t)(breathing_config.max_brightness * brightness_factor);
        
        // 根据亮度调整RGB值
        uint8_t r = (uint8_t)((breathing_config.r * current_brightness) / 100);
        uint8_t g = (uint8_t)((breathing_config.g * current_brightness) / 100);
        uint8_t b = (uint8_t)((breathing_config.b * current_brightness) / 100);
...
void breathing_start(uint32_t cycle_time_s)
{
    ...
    if (breathing_thread != RT_NULL) {
        rt_thread_delete(breathing_thread);
        breathing_thread = RT_NULL;
    }
    
    // 设置周期时间(转换为毫秒)
    if (cycle_time_s < 1) cycle_time_s = 1;
    if (cycle_time_s > 60) cycle_time_s = 60; // 最长60秒
    breathing_config.cycle_time_ms = cycle_time_s * 1000;
...
void breathing_stop(void)
{
    if (breathing_thread != RT_NULL) {
        ...
        // 关闭所有LED
        uint8_t color_data[WS2812_LED_NUM * 3] = {0};
        ws2812_update(color_data);
...
/* MSH命令：停止呼吸灯效果 */
static int cmd_breathing_stop(int argc, char **argv)
{
    breathing_stop();
    return RT_EOK;
}
MSH_CMD_EXPORT(cmd_breathing_stop, stop breathing effect);
