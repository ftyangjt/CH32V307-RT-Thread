...
void lcd_init(void)
{
    ...
    FSMC_NORSRAMInitTypeDef  fsmc_sram_handle;
    ...
    gpio_init_struct.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_14|GPIO_Pin_15;
    ...
    gpio_init_struct.GPIO_Pin = LCD_WR_GPIO_PIN;
    GPIO_Init(LCD_WR_GPIO_PORT, &gpio_init_struct);     /* 初始化LCD_WR引脚 */
    ...
    fsmc_sram_handle.FSMC_WaitSignalPolarity = FSMC_WaitSignalPolarity_Low;         /* 等待信号的极性,仅在突发模式访问下有用 */
    fsmc_sram_handle.FSMC_WaitSignalActive = FSMC_WaitSignalActive_BeforeWaitState; /* 存储器是在等待周期之前的一个时钟周期还是等待周期期间使能NWAIT */
    fsmc_sram_handle.FSMC_WriteOperation = FSMC_WriteOperation_Enable;              /* 存储器写使能 */
    fsmc_sram_handle.FSMC_WaitSignal = FSMC_WaitSignal_Disable;                     /* 等待使能位,此处未用到 */
    fsmc_sram_handle.FSMC_ExtendedMode = FSMC_ExtendedMode_Enable;                  /* 读写使用不同的时序 */
    fsmc_sram_handle.FSMC_AsynchronousWait = FSMC_AsynchronousWait_Disable;         /* 是否使能同步传输模式下的等待信号,此处未用到 */
    fsmc_sram_handle.FSMC_WriteBurst = FSMC_WriteBurst_Disable;                     /* 禁止突发写 */
    fsmc_sram_handle.FSMC_ReadWriteTimingStruct = &fsmc_read_handle;                /* FSMC读时序控制 */
    fsmc_sram_handle.FSMC_WriteTimingStruct = &fsmc_write_handle;                   /* FSMC写时序控制 */
    ...
    {
        /* 重新配置写时序控制寄存器的时序 */
        fsmc_write_handle.FSMC_AddressSetupTime = 3; /* 地址建立时间 */
        fsmc_write_handle.FSMC_DataSetupTime = 3;    /* 数据保持时间 */
    }
    else if (lcddev.id == 0x9806 || lcddev.id == 0x9341 || lcddev.id == 0x5510)
    {
        /* 重新配置写时序控制寄存器的时序 */
        fsmc_write_handle.FSMC_AddressSetupTime = 2; /* 地址建立时间 */
        fsmc_write_handle.FSMC_DataSetupTime = 2;    /* 数据保持时间 */
    }
    else if (lcddev.id == 0x5310 || lcddev.id == 0x7796 || lcddev.id == 0x1963)
    {
        /* 重新配置写时序控制寄存器的时序 */
        fsmc_write_handle.FSMC_AddressSetupTime = 1; /* 地址建立时间 */
