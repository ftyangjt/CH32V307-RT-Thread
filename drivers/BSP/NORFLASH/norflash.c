...
void norflash_init(void)
{
    ...
    gpio_init_struct.GPIO_Pin = NORFLASH_CS_GPIO_PIN;
    gpio_init_struct.GPIO_Mode = GPIO_Mode_Out_PP;
    gpio_init_struct.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(NORFLASH_CS_GPIO_PORT, &gpio_init_struct);    /* CS引脚模式设置(复用输出) */

    NORFLASH_CS(1);                          /* 取消片选 */

    spi2_init();                             /* 初始化SPI2 */
    spi2_set_speed(SPI_SPEED_4);             /* SPI2 切换到高速状态 18Mhz */
