...
#define NORFLASH_CS_GPIO_CLK_ENABLE()   do{ RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE); }while(0)   /* PB口时钟使能 */

/* NORFLASH 片选信号 */
#define NORFLASH_CS(x)              do{ x ? \
                                          GPIO_SetBits(NORFLASH_CS_GPIO_PORT, NORFLASH_CS_GPIO_PIN) : \
                                          GPIO_ResetBits(NORFLASH_CS_GPIO_PORT, NORFLASH_CS_GPIO_PIN); \
                                    }while(0)
